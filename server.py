...
#
# E4B has NOT been measured. Its W4A16 weights are 9.21 GB against E2B's 6.56, so
# scaling E2B's peak by that ratio estimates ~36 GiB — which puts it OVER the
# 32 GiB tier that covers E2B. Treat 64 GiB as the floor for E4B until someone
# measures it, and do not assume the E2B tier carries over.
#
# So the useful tiers are 64 GiB to inspect a 31B parameter tree and 128 GiB to
# actually run it. 32 GiB covers E2B only. Guessing from checkpoint size
...
async def create_cpu_debug_vm(
    ...
    machine_type: Annotated[
        ...
        Field(description="GCE machine type. Memory gates what loads, vCPUs gate how long you wait. "
                          "e2-highmem-16 = 16 vCPU/128 GiB runs a 31B; 64 GiB loads one (and runs E4B); "
                          "32 GiB is E2B only"),
    ] = CPU_DEBUG_MACHINE_TYPE,
    boot_disk_size_gb: Annotated[
        int, Field(ge=50, description="Checkpoints are large: E2B 8.3 GB, E4B 11.5 GB, "
                                      "31B W4A16 23.3 GB, MoE ~14 GB")
    ] = 100,
    spot: Annotated[
    ...
    Memory is the spec that decides what you can debug:
      ...
      * ~64 GiB — E4B (est. ~36 GiB peak, NOT measured — scaled from E2B by the
                  9.21/6.56 GB weight ratio), and enough to LOAD the 31B and
                  inspect its parameter tree (48 GiB measured peak), though a
                  31B forward pass OOMs here.
      * ~128 GiB — enough to RUN the 31B on CPU.
    The measured figures are peak RSS. XLA:CPU allocates roughly 2x what the
    weights occupy, so sizing from checkpoint bytes will leave you short — which
    is also why E4B does not fit the tier E2B fits.
