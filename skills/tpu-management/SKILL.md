...
docker-installing startup script, cloud-platform scopes; `workload="jax"` for a bare
JAX VM instead), `find_tpu_vm` (zone sweep, same workload choice),
`wait_for_vllm_ready` (poll until serving), `wait_for_jax_ready`,
`verify_jax_tpu` (assert jax.devices() sees a TPU), `list_tpu_vm_instances`,
...
**E4B QAT is expected to fail the same way, but this has not been verified.** Both
failures are properties of the E-series QAT export rather than of E2B specifically:
E4B ships the same W4A16 `per_layer_model_projection` and the same KV-sharing
layout, so the second error would name `layers.24-41` instead of `layers.15-34`
(E4B shares KV over its last 18 of 42 layers, E2B over its last 20 of 35). Serve
the plain `google/gemma-4-E4B-it` under vLLM until someone confirms otherwise.

This limitation is vLLM's, not the checkpoint's — the pure-JAX engine in this repo
loads the QAT variants directly, including the W4A16 `per_layer_model_projection`
that vLLM rejects (see `ports/gemma4/jax_e_loader.py` and tpu-inference #3225).

## JAX on TPU (bare dev VMs)

For direct JAX work — kernels, benchmarks, custom inference — instead of vLLM
serving, provision with `workload="jax"`:

1. `create_tpu_vm_instance(workload="jax")` (default name `jax-tpu-vm`) or
